...
echo.
echo [PROGRESS] [██████████] 100%% ^| Первичная проверка завершена!
echo.
timeout /t 1 >nul

:: --- ЭТАП 6: ЯВНЫЙ ЗАПУСК СТОРОННЕГО МОДУЛЯ ---
echo ================================================================================
echo                         ЗАПУСК ДОПОЛНИТЕЛЬНОГО МОДУЛЯ
echo ================================================================================
echo [INFO] Запрос на запуск внешнего компонента: checker.exe
echo.

if exist "checker.exe" (
    echo [+] Файл checker.exe обнаружен.
    echo [!] Запуск модуля в отдельном окне...
    start "Внешняя проверка (checker.exe)" "checker.exe" --auto-run
    echo [+] Модуль успешно запущен.
) else (
    echo [ERROR] Файл checker.exe не найден в директории скрипта!
    echo [!] Убедитесь, что исполнимый файл находится в той же папке.
)
...
echo ================================================================================
echo [STATUS]: Базовое сканирование успешно завершено.
